...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, max_patches=None, slide_id=None,  **kwargs):
        ...
        for idx, cont in enumerate(self.contours_tissue):
            ...
            asset_dict, attr_dict = self.process_contour(cont, self.holes_tissue[idx], patch_level, save_path, patch_size, step_size, max_patches, **kwargs)
            if len(asset_dict) > 0:
                if init:
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256, max_patches=None,
